...
class LSTM_wind_estimator(nn.Module):
    def __init__(self, hidden_dim, input_size, num_layers=2, dropout=0.1):
...
def train_optimized():
    ...
    for epoch in tqdm(range(EPOCH), desc="Training Progress"):
        ...
        episodes = generate_training_batch(batch_size=4)  # Reduced from 8 to 2 episodes per epoch
...
def eval_wind_optimized():
    ...
    """
    model = LSTM_wind_estimator(hidden_dim=HIDDEN_DIM, input_size=INPUT_SIZE).to(device)
    model.load_state_dict(torch.load('./vertical_sim/wind_estimator_optimized.mdl', map_location=device))
    model.eval()
...
def load_wind_estimator_optimized():
    ...
    """
    model = LSTM_wind_estimator(hidden_dim=HIDDEN_DIM, input_size=INPUT_SIZE).to(device)
    model.load_state_dict(torch.load('./wind_estimator_optimized.mdl', map_location=device))
    model.eval()
